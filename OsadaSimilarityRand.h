...
class SD:public ModelBase
{
 public:
    ...
    RArea* calPseudoSampling(Area *AR); // Calculate Sampling random points by pseudo-sampling (not area weighted)
    void pseudoRandpoints2pcd(RArea *RAR); // output pseudo-randpoints by pcd file
	RArea* calsampling(Area *AR); // Calculate Sampling random points by pseudo-sampling (area weighted, so more uniform)
...
// Calculate Sampling random points by pseudo-sampling (area weighted, so more uniform)
